...
export default function PythonTopicPage({
  ...
  nextHref,
  nextLabel,
}: PythonTopicPageProps) {
  return (
    <main className="min-h-screen bg-slate-50 px-6 py-12 text-slate-950">
      <article className="mx-auto max-w-5xl">
        <div className="flex flex-wrap gap-3 text-sm font-medium">
          <Link href="/labs/python" className="text-teal-700">
            go to Python
          </Link>
        ...
        <section className="mt-6 rounded-lg border border-slate-200 bg-white p-5">
          <h2 className="text-xl font-semibold">직접 해보기</h2>
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            {practice.map((item) => (
              <section key={item.title} className="rounded-lg bg-slate-50 p-4">
